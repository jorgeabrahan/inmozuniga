...
window.onload = () => {
    ...
    document.querySelectorAll('.column__link').forEach(footrLnk => {
        footrLnk.addEventListener('click', () => {
            shwCatByName(footrLnk.href.split('#')[1], 0);
        });
    })
    
    if (anchor.match(/\-/g)) shwCatByName(anchor, 2000); 
    ...
    const createPrprtyHtml = (prprtyData, prprtyId, prprtyCnt) => {
        ...
        <div class="property__btn-container">
            <a href="https://inmozuniga.com/propiedad#${prprtyId}" class="property__btn">
                Me interesa
            </a>
        </div>
        `;
    ...
    db.collection('PROPIEDADES').get().then(prprtis => {
        prprtis.forEach(prprty => {
            ...
            if (prprty.data().estado == "venta") sortByCat(cntCatSale, prprty.data().categoria, prprtyCnt);
            else sortByCat(cntCatRent, prprty.data().categoria, prprtyCnt);
